        ...
        <div className='rounded-md config w-[33%] my-shadow config p-5'>
            <div>
                ...
                <div className="flex gap-6 flex-wrap ">
                    {colors.map((val, key) => {
                        return (
                            <div className={`rounded-md border p-2 cursor-pointer ${currentColor == val && 'active'}`} key={key} onClick={() => setCorrentColor(val)}>
                                <div className='w-8 h-8 rounded-md' style={{ background: val }}>
                                </div>
                            </div>)
                    })}
                </div>
